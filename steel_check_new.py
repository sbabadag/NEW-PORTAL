...
def check_deflections(nodes, span, h1, h2, design_code="Eurocode 3 (EN)"):
    ...
    try:
        ...
        # Vertical deflections (y-direction, index 1 in DOF)
        ridge_vertical = 0
        left_beam_vertical = 0
        right_beam_vertical = 0
        ...
        # Horizontal deflections (x-direction, index 0 in DOF)
        left_top_horizontal = 0
        right_top_horizontal = 0
        ridge_horizontal = 0
        
        if hasattr(nodes[1], 'D') and nodes[1].D is not None and len(nodes[1].D) > 0:
            left_top_horizontal = abs(nodes[1].D[0])
        if hasattr(nodes[3], 'D') and nodes[3].D is not None and len(nodes[3].D) > 0:
            right_top_horizontal = abs(nodes[3].D[0])
        ...
        vertical_limit = (span/2) / vertical_limit_factor * 1000  # mm
        h_avg = (h1 + h2) / 2
        horizontal_limit = h_avg / horizontal_limit_factor * 1000  # mm
        
        # Convert deflections to mm
        ...
        left_top_horizontal_mm = left_top_horizontal * 1000
        right_top_horizontal_mm = right_top_horizontal * 1000
        ...
        max_horizontal_deflection = max(left_top_horizontal_mm, right_top_horizontal_mm, ridge_horizontal_mm)
        ...
        detailed_calculations = {
            "vertical": {
                "formula": f"δ_max ≤ L/{vertical_limit_factor}",
                "calculation": f"δ_limit = {span/2:.1f} m / {vertical_limit_factor} = {vertical_limit:.1f} mm",
                "measured": f"δ_max = max({ridge_vertical_mm:.1f}, {left_beam_vertical_mm:.1f}, {right_beam_vertical_mm:.1f}) = {max_vertical_deflection:.1f} mm",
            ...
            "horizontal": {
                "formula": f"δ_max ≤ H/{horizontal_limit_factor}",
                "calculation": f"δ_limit = {h_avg:.1f} m / {horizontal_limit_factor} = {horizontal_limit:.1f} mm",
                "measured": f"δ_max = max({left_top_horizontal_mm:.1f}, {right_top_horizontal_mm:.1f}, {ridge_horizontal_mm:.1f}) = {max_horizontal_deflection:.1f} mm",
...
def optimize_portal_frame_total_weight(*args, **kwargs):
    return {"error": "Optimization not implemented"}
